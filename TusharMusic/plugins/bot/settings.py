...
ButtonList = List[List[InlineKeyboardButton]]

def empty_markup() -> ButtonList:
    """Return an empty button list as a safe default"""
    return [[]]
...
async def without_Admin_rights(client, CallbackQuery, _):
    ...
    if command == "PM":
        ...
        msg = _["setting_5"]
        msg += _["setting_6"]
        msg += _["setting_7"].format(playmode)
        upl = cast(ButtonList, playmode_users_markup(_))
    elif command == "AU":
        current = await get_authuser_names(CallbackQuery.message.chat.id)
        msg = _["auth_1"]
        msg += _["auth_2"].format(OWNER_ID)
        msg += _["auth_3"].format(current)
        msg += _["auth_8"]
        upl = cast(ButtonList, auth_users_markup(_))
    elif command == "VM":
        mode = await is_skipmode(CallbackQuery.message.chat.id)
        current = await get_upvote_count(CallbackQuery.message.chat.id)
        buttons = cast(ButtonList, vote_mode_markup(_, current, bool(mode)))

    markup = upl if command in ["PM", "AU"] else buttons
    try:
        return await CallbackQuery.edit_message_reply_markup(
...
async def playmode_ans(client, CallbackQuery, _):
    ...
    buttons = cast(ButtonList, playmode_users_markup(_, Direct, Group, Playtype))
    try:
        return await CallbackQuery.edit_message_reply_markup(
            reply_markup=InlineKeyboardMarkup(buttons)
        )
...
async def authusers_mar(client, CallbackQuery, _):
    ...
    if command == "AUTHLIST":
        ...
        for note in _authusers:
            auth_data = await get_authuser(CallbackQuery.message.chat.id, note)
            if not isinstance(auth_data, dict):
                continue

            user_id = auth_data.get("auth_user_id")
            ...
            try:
                ...
                if isinstance(user, User):
                    ...
                    j += 1
                    msg += f"{j}➤ {username}[<code>{user_id}</code>]\n"
    ...
    if command == "AUTH":
        is_non_admin = await is_nonadmin_chat(CallbackQuery.message.chat.id)
        buttons = cast(ButtonList, auth_users_markup(_, not is_non_admin))
...
async def vote_change(client, CallbackQuery, _):
    ...
    current = await get_upvote_count(CallbackQuery.message.chat.id)
    buttons = cast(ButtonList, vote_mode_markup(_, current, mode))
